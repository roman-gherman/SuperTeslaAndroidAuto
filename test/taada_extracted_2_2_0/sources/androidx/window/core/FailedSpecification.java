...
final class FailedSpecification<T> extends SpecificationComputer<T> {
    ...
    public FailedSpecification(T value, String tag, String message, Logger logger, SpecificationComputer.VerificationMode verificationMode) {
        ...
        if (length < 0) {
            throw new IllegalArgumentException(b.d(length, "Requested element count ", " is less than zero.").toString());
        }
        if (length == 0) {
            collectionP = u.f3805a;
        } else {
            int length2 = stackTrace.length;
            if (length >= length2) {
                collectionP = j.L(stackTrace);
            } else if (length == 1) {
                collectionP = Z.p(stackTrace[length2 - 1]);
            } else {
        ...
        Object[] array = collectionP.toArray(new StackTraceElement[0]);
        if (array == null) {
