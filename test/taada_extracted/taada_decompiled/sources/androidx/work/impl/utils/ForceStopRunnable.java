            ...
            if (i >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    ...
                    for (int i3 = 0; i3 < historicalProcessExitReasons.size(); i3++) {
                        ApplicationExitInfo applicationExitInfoD = c.d(historicalProcessExitReasons.get(i3));
                        if (applicationExitInfoD.getReason() == 10 && applicationExitInfoD.getTimestamp() >= lastForceStopEventMillis) {
                            return true;
                        }
                    }
                }
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (i >= 3) {
                                ...
                                long j6 = ((long) i) * BACKOFF_DURATION_MS;
                                Logger.get().debug(TAG, "Retrying after " + j6, e);
                                sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
                            }
                        }
                        long j62 = ((long) i) * BACKOFF_DURATION_MS;
                        Logger.get().debug(TAG, "Retrying after " + j62, e);
                        sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
