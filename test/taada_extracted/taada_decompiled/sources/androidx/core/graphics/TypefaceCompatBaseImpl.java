...
class TypefaceCompatBaseImpl {
    ...
    public Typeface createWeightStyle(Context context, Typeface typeface, int i, boolean z6) {
        ...
        try {
            typefaceCreateWeightStyle = WeightTypefaceApi14.createWeightStyle(this, context, typeface, i, z6);
        } catch (RuntimeException unused) {
    ...
    private FontResourcesParserCompat.FontFileResourceEntry findBestEntry(FontResourcesParserCompat.FontFamilyFilesResourceEntry fontFamilyFilesResourceEntry, int i, boolean z6) {
        return (FontResourcesParserCompat.FontFileResourceEntry) findBestFont(fontFamilyFilesResourceEntry.getEntries(), i, z6, new StyleExtractor<FontResourcesParserCompat.FontFileResourceEntry>() { // from class: androidx.core.graphics.TypefaceCompatBaseImpl.3
            @Override // androidx.core.graphics.TypefaceCompatBaseImpl.StyleExtractor
            public int getWeight(FontResourcesParserCompat.FontFileResourceEntry fontFileResourceEntry) {
                return fontFileResourceEntry.getWeight();
            }
    ...
    private static <T> T findBestFont(T[] tArr, int i, boolean z6, StyleExtractor<T> styleExtractor) {
        T t6 = null;
        int i3 = Integer.MAX_VALUE;
        for (T t7 : tArr) {
            int iAbs = (Math.abs(styleExtractor.getWeight(t7) - i) * 2) + (styleExtractor.isItalic(t7) == z6 ? 0 : 1);
            if (t6 == null || i3 > iAbs) {
                t6 = t7;
                i3 = iAbs;
            }
    ...
    public Typeface createFromFontFamilyFilesResourceEntry(Context context, FontResourcesParserCompat.FontFamilyFilesResourceEntry fontFamilyFilesResourceEntry, Resources resources, int i, boolean z6) {
        FontResourcesParserCompat.FontFileResourceEntry fontFileResourceEntryFindBestEntry = findBestEntry(fontFamilyFilesResourceEntry, i, z6);
        if (fontFileResourceEntryFindBestEntry == null) {
            return null;
        }
