...
final class Constants {
    ...
    public static void checkAsmExperimental(Object obj) {
        ...
        String strReplace = cls.getName().replace(TypePool.Default.LazyTypeDescription.GenericTypeToken.INNER_CLASS_PATH, ClassPathElement.SEPARATOR_CHAR);
    ...
    public static boolean isWhitelisted(String str) {
        if (str.startsWith("net/bytebuddy/jar/asm/")) {
            return str.contains("Test$") || Pattern.matches("net/bytebuddy/jar/asm/util/Trace(Annotation|Class|Field|Method|Module|RecordComponent|Signature)Visitor(\\$.*)?", str) || Pattern.matches("net/bytebuddy/jar/asm/util/Check(Annotation|Class|Field|Method|Module|RecordComponent|Signature)Adapter(\\$.*)?", str);
        }
        return false;
    }
}
