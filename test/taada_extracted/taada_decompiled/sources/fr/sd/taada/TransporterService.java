...
public class TransporterService extends Service {
    ...
    public int onStartCommand(Intent intent, int i, int i3) {
        ...
        try {
            getLogManager().logInfo(TAG, "[TIMING] Launching Android Auto activity");
            startActivity(intent2);
            getLogManager().logInfo(TAG, "[TIMING] Android Auto activity started - waiting for connection on port 5288");
            Integer.parseInt(defaultSharedPreferences.getString("car_type", "0"));
            return 1;
        } catch (Exception e6) {
            getLogManager().logError(TAG, "❌ Failed to start Android Auto: " + e6.getMessage(), e6);
            Toast.makeText(this, getText(R.string.noaa), 1).show();
            HashMap map2 = new HashMap();
            map2.put("error", e6.getMessage());
            logSessionEnd("auto_start_failed", map2);
